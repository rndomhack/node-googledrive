...
class GooleDrive extends EventEmitter {
    ...
    initialize() {
        ...
        const clientId = this._client.installed.client_id;
        const clientSecret = this._client.installed.client_secret;
        const redirectUri = this._client.installed.redirect_uris[0];
    ...
    checkToken() {
        if (Date.now() + 60 * 1000 > this._token.expiry_date) {
