...
class GooleDrive {
    ...
    authorize() {
        ...
        this.settings.drive = google.drive({ version: "v2", auth: this.settings.oAuth2 });
    ...
    getRootFolder() {
        return this.checkToken().then(() => {
            return new Promise((resolve, reject) => {
                this.settings.drive.about.get({}, (err, res) => {
                    if (err) {
                        reject(err);
                        return;
                    }

                    resolve(res.rootFolderId);
                });
            });
        }).then(rootFolderId => {
            var rootFolder = new googleDriveFs.Folder({
                id: rootFolderId,
                context: this
            });

            return rootFolder.initialize().then(() => rootFolder);
        });
    }
